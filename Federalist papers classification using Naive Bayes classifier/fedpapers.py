...
fH = (np.sum(XH,axis=0) + 1)
fH = [float(fH[i])/(np.sum(XH) + d) for i in range(len(fH))]

# Estimate probability of each word in vocabulary being used by Madison
fM = (np.sum(XM,axis=0) + 1)
fM = [ float(fM[i])/(np.sum(XM) + d) for i in range(len(fM)) ]

# Compute ratio of these probabilities
fratio = [ float(fH[i]/fM[i]) for i in range(len(fH)) ]

# Compute prior probabilities 
piH = float(XH.shape[0])/(XH.shape[0] + XM.shape[0])
piM = float(XM.shape[0])/(XH.shape[0] + XM.shape[0])
...
print "count_Hamilton: %d " %  count_H
print "count_Madison: %d " %  count_M
